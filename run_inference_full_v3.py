"""
================================================================================
 Spider 2.0-Lite Text-to-SQL 全量推理引擎 V3 — Schema Catalog + RAG 双通道
================================================================================
与 V2 的区别:
  - BigQuery/Snowflake/SQLite 任务: 额外注入 EK Retriever 检索到的知识片段
  - 新增 --no-rag 参数: 禁用 RAG，回退到 V2 行为（A/B 对比用）
...
def _get_log_file():
    ...
    if _LOG_FILE is None:
        _LOG_FILE = open(BASE_DIR / "inference_log_v3.txt", "w", encoding="utf-8")
    return _LOG_FILE
...
def main(limit: Optional[int] = None, use_catalog: bool = True,
         use_rag: bool = True) -> None:
    """主入口"""

    log("INFO", "=" * 60)
    catalog_str = "ON" if use_catalog else "OFF"
    rag_str = "ON" if use_rag else "OFF"
    log("INFO", f"Spider 2.0-Lite 全量推理引擎 V3 启动 (Schema Catalog={catalog_str}, RAG={rag_str})")
    log("INFO", "=" * 60)
    ...
    if _LOG_FILE is not None:
        log("INFO", f"日志已保存到 {BASE_DIR / 'inference_log_v3.txt'}")
        _LOG_FILE.close()
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Spider 2.0-Lite 全量推理引擎 V3")
    parser.add_argument("--limit", type=int, default=None,
                        help="限制处理的任务数（测试用）")
    parser.add_argument("--no-catalog", action="store_true",
                        help="禁用 Schema Catalog，回退到 V1 行为")
    parser.add_argument("--no-rag", action="store_true",
                        help="禁用 RAG，回退到 V2 行为")
