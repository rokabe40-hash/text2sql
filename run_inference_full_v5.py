"""
================================================================================
 Spider 2.0-Lite Text-to-SQL 推理引擎 V5 — RAG 优化版
================================================================================
V5 相对 V4 的改动:
  - RAG 分数阈值: min_score=0.600，过滤低分无关检索
  - 消除双重搜索: search_and_format() 一次调用完成检索+格式化+日志
  - 移除低质量文档: licenses.md, music_length_type.md 已排除
  - 输出目录: my_predicted_sqls_v5/

使用:
  SQLite 快速测试:  python run_inference_full_v5.py --sample-sqlite 30
  全量 SQLite:      python run_inference_full_v5.py --sample-sqlite 135
  A/B 对照(无RAG):  python run_inference_full_v5.py --sample-sqlite 135 --no-rag
================================================================================
"""
...
OUTPUT_DIR = BASE_DIR / "my_predicted_sqls_v5"
...
def _get_log_file():
    ...
    if _LOG_FILE is None:
        _LOG_FILE = open(BASE_DIR / "inference_log_v5.txt", "w", encoding="utf-8")
    return _LOG_FILE
...
def main(limit: Optional[int] = None, use_catalog: bool = True,
    ...
    log("INFO", f"Spider 2.0-Lite V5 (RAG Optimized) "
    ...
    for idx, task in enumerate(tasks, start=1):
        ...
        # RAG retrieval (V5: search_and_format, single encode call)
        ek_rag_context = ""
        if retriever and question:
            query_text = f"{db_name}: {question}"
            try:
                ek_rag_context, raw_results = retriever.search_and_format(
                    query_text, top_k=3, max_chars=2500, min_score=0.600
                )
                if ek_rag_context:
                    rag_hits += 1
                    log("INFO", f"  ->RAG hit: {raw_results[0]['header_path']} "
                           f"(score={raw_results[0]['score']:.3f})")
            except Exception as exc:
    ...
    if _LOG_FILE is not None:
        log("INFO", f"Log saved to {BASE_DIR / 'inference_log_v5.txt'}")
        _LOG_FILE.close()


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Spider 2.0-Lite V5 (RAG Optimized)")
