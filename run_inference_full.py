""""
================================================================================
 Spider 2.0-Lite Text-to-SQL 全量推理引擎（547 任务 / SQLite + BigQuery + Snowflake）
================================================================================
功能：
  1. 读取 spider2-lite.jsonl 中的全部 547 个任务
  2. 根据 instance_id 前缀自动识别平台：
       localXXX   → SQLite（提取本地 .sqlite 文件 Schema）
       bqXXX      → Google BigQuery（基于公开数据集知识生成）
       sf_bqXXX   → Snowflake（基于公开数据集知识生成）
       gaXXX      → BigQuery GA 数据集
  3. 对 SQLite 任务：动态提取 CREATE TABLE 语句作为 Schema
  4. 对云任务：使用平台对应的 SQL 方言 Prompt（无本地 Schema，依赖 LLM 先验知识）
...
def _get_log_file():
    ...
    if _LOG_FILE is None:
        _LOG_FILE = open(BASE_DIR / "inference_log.txt", "w", encoding="utf-8")
    return _LOG_FILE
...
def main(limit: Optional[int] = None) -> None:
    ...
    # ---------- 启动检查 ----------
    log("INFO", "=" * 60)
    log("INFO", "Spider 2.0-Lite 全量推理引擎启动 (SQLite + BigQuery + Snowflake)")
    log("INFO", "=" * 60)
    ...
    for idx, task in enumerate(tasks, start=1):
        ...
        # ================================================================
            ...
            if ek_content:
                ...
                log("INFO", f"  ->无外部知识文件，依赖 LLM 对 {db_name} 的先验知识")
    ...
    if _LOG_FILE is not None:
        log_path = BASE_DIR / "inference_log.txt"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Spider 2.0-Lite 全量推理引擎")
